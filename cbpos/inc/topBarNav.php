<nav class="navbar navbar-expand navbar-dark bg-gradient-pink">
  <div class="container px-3 px-lg-5">
    <div class="d-flex align-items-center w-100">
      <!-- Logo -->
      <a class="navbar-brand mr-2" href="./">
        <img src="<?php echo validate_image($_settings->info('logo')) ?>" width="30" height="30" class="d-inline-block align-top" alt="" loading="lazy">
        <?php echo $_settings->info('short_name') ?>
      </a>

      <!-- Search Bar (flexible, always visible) -->
      <form class="form-inline flex-grow-1 mx-2" id="search-form">
        <div class="input-group w-100">
          <input class="form-control form-control-sm" type="search" placeholder="Search" aria-label="Search" name="search" value="<?php echo isset($_GET['search']) ? $_GET['search'] : "" ?>">
          <div class="input-group-append">
            <button class="btn btn-outline-light btn-sm m-0" type="submit"><i class="fa fa-search"></i></button>
          </div>
        </div>
      </form>

      <!-- Cart badge – visible only on mobile (hidden on lg and up) -->
      <?php if($_settings->userdata('id') > 0 && $_settings->userdata('login_type') == 2): ?>
      <a class="nav-link text-white d-lg-none p-0" href="./?p=cart" aria-label="Cart">
        <i class="bi-cart-fill"></i>
        <span class="badge bg-dark text-white rounded-pill" id="cart-count-mobile">
          <?php 
            $count = $conn->query("SELECT SUM(quantity) as items FROM `cart` WHERE client_id = ".$_settings->userdata('id'))->fetch_assoc()['items'];
            echo ($count > 0 ? $count : 0);
          ?>
        </span>
      </a>
      <?php else: ?>
      <button class="btn btn-outline-dark btn-sm ml-2 d-lg-none" id="login-btn" type="button">Login</button>
      <?php endif; ?>

      <!-- Account / Login area – visible only on larger screens (lg and up) -->
      <div class="d-none d-lg-flex align-items-center ml-2">
        <?php if($_settings->userdata('id') > 0 && $_settings->userdata('login_type') == 2): ?>
          <a class="nav-link text-white mr-2" href="./?p=cart">
            <i class="bi-cart-fill me-1"></i> Cart
            <span class="badge bg-dark text-white ms-1 rounded-pill" id="cart-count">
              <?php echo ($count > 0 ? $count : 0); ?>
            </span>
          </a>
          <a href="./?p=my_account" class="nav-link text-white"><b>Hi, <?php echo $_settings->userdata('firstname')?>!</b></a>
          <a href="logout.php" class="nav-link text-white"><i class="fa fa-sign-out-alt"></i></a>
        <?php else: ?>
          <button class="btn btn-outline-dark ml-2" id="login-btn-desktop" type="button">Login</button>
        <?php endif; ?>
      </div>
    </div>
  </div>
</nav>

<script>
  $(function(){
    // Login modal trigger
    $('#login-btn, #login-btn-desktop').click(function(){
      uni_modal("","login.php")
    })

    // Search form submit
    $('#search-form').submit(function(e){
      e.preventDefault()
      var sTxt = $('[name="search"]').val()
      if(sTxt != '')
        location.href = './?p=products&search='+sTxt;
    })
  })
</script>